...
from keras.layers.core import Activation, Flatten, Dropout, Dense, Reshape, Lambda
...
def get_center_pixel( x ):
    return x[:,8,8,:]

def NDWI( x ):
  NDW_idx = np.divide((x[:,2]-x[:,7]),(x[:,2]+x[:,7]))
  # return Reshape((1))(NDW_idx)
  return K.expand_dims(NDW_idx, axis = -1)

def NDVI( x ):
  NDV_idx = np.divide((x[:,7]-x[:,3]),(x[:,3]+x[:,7]))
  return K.expand_dims(NDV_idx, axis = -1)

def NDMI( x ):
  NDM_idx = np.divide((x[:,5]-x[:,7]),(x[:,5]+x[:,7]))
  return K.expand_dims(NDM_idx, axis = -1)

def NDWI2( x ):
  NDW2_idx = np.divide((x[:,7]-x[:,0]),(x[:,7]+x[:,0]))
  return K.expand_dims(NDW2_idx, axis = -1)
...
def inception_m_end( input_net, num_classes = 7, first_layer = None ):
    ...
    flat = Dense(64, kernel_regularizer=l2(0.0002))(flat)
    flat = Dropout(0.4)(flat)
    if first_layer is not None:
        print('Input tensor shape: ', first_layer.shape)
        (batch_size, x_size, y_size, channels_size) = first_layer.shape
        center_pixel = Lambda(get_center_pixel)(first_layer)
        pixel_NDWI = Lambda(NDWI)(center_pixel)
        pixel_NDVI = Lambda(NDVI)(center_pixel)
        pixel_NDMI = Lambda(NDMI)(center_pixel)
        pixel_NDWI2 = Lambda(NDWI2)(center_pixel)
        center_pixel = Concatenate(axis = 1)([center_pixel, pixel_NDWI, pixel_NDVI,
                                                pixel_NDMI, pixel_NDWI2])
        # center_pixel = Reshape((1, channels_size))(center_pixel)
        # print('Center pixel tensor shape: ', center_pixel.shape)
        print('Center pixel tensor: ', center_pixel)
        # center_pixel = Flatten()(center_pixel)
        center_pixel = Dense(144, kernel_regularizer=l2(0.0002))(center_pixel)
        center_pixel = Dropout(0.2)(center_pixel)
        center_pixel = Dense(128, kernel_regularizer=l2(0.0002))(center_pixel)
        center_pixel = Dropout(0.2)(center_pixel)
        center_pixel = Dense(64, kernel_regularizer=l2(0.0002))(center_pixel)
        center_pixel = Dropout(0.2)(center_pixel)
        flat = Concatenate(axis = -1)([center_pixel, flat])
    flat = Dense(128, kernel_regularizer=l2(0.0002))(flat)
...
if __name__ == '__main__':
    ...
    if (reset_model or not model_exist):
        ...
        # IMAGE_PIXEL_DIMS = (1, 1, CHANNELS)
        # BATCH_PIXEL_DIMS = (None, 1, 1, CHANNELS)
        # my_input_pixel = Input( shape=IMAGE_PIXEL_DIMS, batch_shape=BATCH_PIXEL_DIMS )
