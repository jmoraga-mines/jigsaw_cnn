...
def to_rgb( s_img):
  ...
  (x_dim, y_dim) = s_img.shape
  Red = (s_img & 1)*255
  Green = ((s_img & 2)/2)*255
  Blue = ((s_img & 4)/4)*255
  # rgb = np.concatenate((Red.reshape(3182,4571,1),Green.reshape(3182,4571,1),Blue.reshape(3182,4571,1)), axis=2).astype(np.uint8)
  rgb = np.concatenate((Red.reshape(x_dim,y_dim,1),
                        Green.reshape(x_dim,y_dim,1),
                        Blue.reshape(x_dim,y_dim,1)), axis=2).astype(np.uint8)
...
if __name__ == '__main__':
    ...
    # sentinel_image = sentinel_image[:,1200:-500,2700:]
